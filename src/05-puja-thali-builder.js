...
export function setupRemoveButton(button, thaliElement) {
  ...
  const handler = () => {
    if (thaliElement.lastChild) thaliElement.removeChild(thaliElement.lastChild)
  }
  button.addEventListener("click", handler)
  return () => button.removeEventListener("click", handler)
}
...
export function setupToggleItem(button, thaliElement, itemName) {
  ...
  const handler = () => {
    ...
    if (existing) thaliElement.removeChild(existing)
    else {
      const li = document.createElement("li")
      li.textContent = itemName
      thaliElement.appendChild(li)
...
export function createThaliManager(thaliElement, counterElement) {
  ...
  const updateCounter = () => { counterElement.textContent = thaliElement.children.length }
  return {
    addItem(name) {
      const li = document.createElement("li")
      li.textContent = name
    ...
    removeItem(name) {
      const li = Array.from(thaliElement.children).find(li => li.textContent === name)
      if (li) {
