...
export function validateDate(dateString) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateString)) return { valid: false, error: "Date YYYY-MM-DD format mein honi chahiye" }
  const inputDate = new Date(dateString)
  if (isNaN(inputDate.getTime())) return { valid: false, error: "Date YYYY-MM-DD format mein honi chahiye" }
  const today = new Date()
  today.setHours(0, 0, 0, 0)
  inputDate.setHours(0, 0, 0, 0)
  if (inputDate < today) return { valid: false, error: "Date aaj ya future ki honi chahiye" }
  return { valid: true, error: null }
}
